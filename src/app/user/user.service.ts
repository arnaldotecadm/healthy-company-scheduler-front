...
export class UserService {
  ...
  addSoftware(usuarioId: string, softwareId: string) {
    return this.http.post<Usuario>(API + '/user-management/add-software', {
      usuarioId,
      softwareId,
    });
  }
  ...
  removeSoftware(usuarioId: string, softwareId: string) {
    return this.http.post<Usuario>(API + '/user-management/remove-software', {
      usuarioId,
      softwareId,
    });
  }
  ...
  removeById(usuarioId: number) {
    return this.http.delete<Usuario[]>(
      API + '/user-management/remove/' + usuarioId
    );
  }
}
